...
如果你的频道走真诚路线（健康科普通常是），SAT 给 3 当占位即可。
...
跑完 5 篇后，你的 baseline 已经从 5 个数据点稳定。`/cheat-status` 会主动提示可跑 `/cheat-bump --bucket-only` 自动派生固定绝对 bucket 边界。

### N≥10 之后：可以转向 percentile 桶

校准池 ≥ 10 时，bucket 边界改用 percentile：你前 20% 的视频是"爆款"，10-20% 是"命中"，30-70% 是"基础盘"。
...
**你不应该做的事**：基于 cold-start 期的 composite 决定要不要发某条稿子。**精度 ±50%——决策没意义**。
...
**cold-start 期追求"信息量最大"，稳定期再追求"全发爆款"**。
...
### 路径 B：直接采纳 v2 作为起点
把 opinion-video.md 的 v2 公式抄到你的 `rubric_notes.md`，**然后跑 `/cheat-bump`** 验证。

### 路径 C：保持 v0 等权，跑更多样本
如果 5 篇还看不出明显 pattern，再跑 5 篇。
...
cold-start 期最大的诱惑是"看着 composite 8.4 就觉得这条会爆"。**别信**。第 5 篇之后再开始相信数字。
